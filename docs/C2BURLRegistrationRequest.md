...
------------ | ------------- | ------------- | -------------
**short_code** | **int** | Usually, a unique number is tagged to an M-PESA pay bill/till number of the organization. | [optional] 
...
**confirmation_url** | **str** | This is the URL that receives the confirmation request from API upon payment completion. | [optional] 
**validation_url** | **str** | This is the URL that receives the validation request from the API upon payment submission. The validation URL is only called if the external validation on the registered shortcode is enabled. (By default External Validation is disabled). | [optional] 
